      ...
      <div class="row">
         <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
               <h4 class="mb-sm-0 font-size-18">Bonanza Transactions</h4>
         ...
         <div class="col-12">
            <div class="card">
               <div class="card-body">
                  <div class="row mb-4">
                     <div class="col-md-8">
                        <h4 class="card-title">Bonanza Transactions</h4>
                     </div>
                  ...
                  <div class="col-12" style="overflow-x:auto;">                     
                     <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        <tbody>
                           @foreach($bonanzas as $key => $data)
                           <tr>
                              <td>{{ $key + $bonanzas->firstItem() }}</td>
                              <td>{{$data->bonanza_transaction_id}}</td>
                              <td>{{number_formats($data->bonanza_value)}}</td>
                              <td>{{$data->bonanza_value_side == "R"? "RIGHT":"LEFT"}}</td>
                              <td>{{canvert_date($data->created_at)}}</td>
                              <td>{{ $data->bonanza_description }}</td>
                              <td>
                                 @if($data->bonanza_type == '1')
                     ...
                     {{ $bonanzas->links('pagination::bootstrap-5') }}    
                  </div>
               </div>
            </div>
         </div>
